...
const Settings = struct {
    spawn_dist: c_int = 32,
    ow_structure_dist_max: c_int = 64,
    bastion_dist: c_int = 96,
    fortress_dist: c_int = 256,
    // require_enchanted_golden_apple: bool = false,
};
...
fn isGoodPyramidLoot(seed: u64, x: c_int, z: c_int, settings: Settings) bool {
    var loot = common.getDesertPyramidLoot(seed, x, z, common.DESERT_PYRAMID_SALT_1_16);

    // if (settings.require_enchanted_golden_apple and loot.enchanted_golden_apples == 0) return false;
    _ = settings;
...
fn checkLower48(seed: u64, settings: Settings) StructureSeedCheckResult {
    ...
    if (0 == cubiomes.getStructurePos(cubiomes.Desert_Pyramid, cubiomes.MC_1_16_1, seed, 0, 0, &main_pos)) return FAIL_RESULT;
    if (@abs(main_pos.x - rp_pos.x) > settings.ow_structure_dist_max or @abs(main_pos.z - rp_pos.z) > settings.ow_structure_dist_max) return FAIL_RESULT;
    // The structures need different biomes, so if they are too close, it's less likely to find valid biomes
    ...
    if (!isGoodPyramidLoot(seed, main_pos.x, main_pos.z, settings)) return FAIL_RESULT;
...
fn checkSister(seed: u64, ssr: StructureSeedCheckResult, settings: Settings) bool {
    ...
    // Check temple valid
    if (0 == cubiomes.isViableStructurePos(cubiomes.Desert_Pyramid, &g, main_pos.x, main_pos.z, 0)) return false;
    ...
    // Check for tree biome around rp
    var trees_found = false;
    const diff: c_int = 20;
    var fx: c_int = rp_pos.x - diff;
    var fz: c_int = rp_pos.z - diff;
    outer: while (fx <= rp_pos.x + diff) : (fx += diff) {
        while (fz <= rp_pos.z + diff) : (fz += diff) {
