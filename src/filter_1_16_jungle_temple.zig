...
const Settings = struct {
    spawn_dist: c_int = 32,
    ow_structure_dist_max: c_int = 32,
...
fn isGoodPyramidLoot(seed: u64, x: c_int, z: c_int, needs_food: bool, extra_iron: c_int, settings: Settings) bool {
    _ = settings;
    var loot = common.getJunglePyramidLoot(seed, x, z, common.JUNGLE_PYRAMID_SALT_1_16);
    loot.iron += @intCast(extra_iron);

    if (needs_food and loot.flesh < 16) return false;
...
fn checkLower48(seed: u64, settings: Settings) StructureSeedCheckResult {
    ...
    if (rp_pos.x > 224 or rp_pos.z > 224) return FAIL_RESULT;

    var main_pos: Pos = .{ .x = 0, .z = 0 };
    if (0 == cubiomes.getStructurePos(cubiomes.Jungle_Temple, cubiomes.MC_1_16_1, seed, 0, 0, &main_pos)) return FAIL_RESULT;
    if ((@abs(main_pos.x - rp_pos.x) + @abs(main_pos.z - rp_pos.z)) > settings.ow_structure_dist_max) return FAIL_RESULT;
    // Please do not have intercourse
    if (main_pos.x == rp_pos.x and main_pos.z == rp_pos.z) return FAIL_RESULT;

    var sv: common.StructureVariantZig = .{};
    if (0 == cubiomes.getVariant(@ptrCast(&sv), cubiomes.Ruined_Portal, cubiomes.MC_1_16_1, seed, rp_pos.x, rp_pos.z, cubiomes.jungle)) return FAIL_RESULT;
    ...
    // Check loot
    const rp_loot = common.getRuinedPortalLoot(seed, rp_pos.x, rp_pos.z, common.RUINED_PORTAL_SALT_1_16);
    if (rp_loot.flint_and_steels < 1 and rp_loot.fire_charges < 1 and rp_loot.flint < 1) return FAIL_RESULT;
    ...
    const needs_food = !(rp_loot.enchanted_golden_apples > 0 or rp_loot.golden_apples > 0 or rp_loot.golden_carrots > 0);
    var rp_iron: c_int = @intCast(@divFloor(rp_loot.iron_nuggets, 9));
    if (rp_loot.flint_and_steels > 0 or rp_loot.fire_charges > 0) rp_iron += 1;
    if (!isGoodPyramidLoot(seed, main_pos.x, main_pos.z, needs_food, rp_iron, settings)) return FAIL_RESULT;
...
fn checkSister(seed: u64, ssr: StructureSeedCheckResult, settings: Settings) bool {
    ...
    // Check temple valid
    if (0 == cubiomes.isViableStructurePos(cubiomes.Jungle_Temple, &g, main_pos.x, main_pos.z, 0)) return false;

    // Check rp biome
    const rp_biome = cubiomes.getBiomeAt(&g, 4, (rp_pos.x >> 2) + 2, 255, (rp_pos.z >> 2) + 2);
    switch (rp_biome) {
        cubiomes.jungle,
        cubiomes.jungle_edge,
        cubiomes.modified_jungle,
        cubiomes.modified_jungle_edge,
        cubiomes.bamboo_jungle,
        => {},
        else => return false,
    }
